...
import { deleteUser, getUsers, getUserStatus, patchUserAdmin, patchUserRestore } from "../../redux/actions";
...
>>>>>>> .merge_file_mTf52l
  ...
  const handleClickAdmin = (e, id, isAdmin, first_name) => {
    e.preventDefault();
    Swal.fire({
      title: `${isAdmin ? 'Esta seguro de eliminar como administrador ' : 'Esta seguro que desea hacer administrador' } a ${first_name}?`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        const token = await getAccessTokenSilently();
        let res = await dispatch(patchUserAdmin(id, { is_admin: !isAdmin }, token));
        Swal.fire(
          `Usuario: ${id} | ${first_name}.`,
          `${res.data.is_admin ? 'Ahora es administrador!' : 'Dejo de ser administrador!' }`,
          'success'
        )
      }
    })
  ...
  const handleClickStatus = async (e, id, name, status) => {
    ...
    if(status) {
      ...
      Swal.fire(
        `Se restauro el usuario ${name}`,
        ``,
        'success'
      )
    } else {
      Swal.fire(
        `El usuario ${name} ya esta activo`,
        ``,
        'error'
      )
    }
                  ...
                  users.map((u) => {
                    return (
                      <tr key={"usersList" + u.first_name}>
                        ...
                        <td onClick={ (e) => handleClickAdmin(e, u.id, u.is_admin, u.first_name) }>
                          {u.is_admin ? (
                        ...
                        <td onClick={ (e) => handleClickStatus(e, u.id, u.first_name, u.destroyTime) }>
                          {!u.destroyTime ? (
                            <div className={s.fl_table_true}>true</div>
                          ) : (
                            <div className={s.fl_table_false}>false</div>
